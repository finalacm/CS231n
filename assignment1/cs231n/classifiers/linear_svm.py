...
def svm_loss_vectorized(W, X, y, reg):
  ...
  loss_matrix = np.maximum(0, scores - correct_class_scores + 1)
  ...
  # loss.                                                                     #
  #############################################################################
  # Consider the matrix multiplication, just construct a matrix.
  # dW[:, j] = sigma((loss_matrix[i][j] > 0) * X[i].T) {1 <= i <= N} + 
  #    sigma((loss_matrix[i][k] > 0) {1 <= k <= C}) * -X[i].T {y[i] == j}
